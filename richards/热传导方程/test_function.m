...
%left=0,right=1,T=1,h_partition=0.1,t_partition=0.1,Gauss_point_number=4
...
for i=1:N_partition+1
    ...
    result1_1(2*i,1)=dfunction_initial(M_partition(i));
end
...
%通过循环得到每一层的结果
for j=2:t_number+1
    result(:,j)=local_richards_solver(t_partition,N_partition,M_partition,T_partition,M_basis,T_basis,Gauss_coefficient_reference_1D,...
        Gauss_point_reference_1D,number_of_elements,number_of_trial_local_basis,number_of_test_local_basis,matrix_size,vector_size,j-1,result(:,j-1));
end
...
exact_solution=sind(360*z).*exp(-t);
