...
boundary_nodes(1,:)=-1;
for k=N1_basis+2:N1_basis+N2_basis
    boundary_nodes(1,k)=-2;
end
for k=2*N1_basis+N2_basis+2:nbn
    boundary_nodes(1,k)=-2;
end
...
boundary_edges(1,:)=-1;
for k=N1_partition+2:N1_partition+N2_partition
    boundary_edges(1,k)=-2;
end
for k=2*N1_partition+N2_partition+2:nbe
    boundary_edges(1,k)=-2;
end
...
for k=1:N1_partition
    boundary_edges(2,k)=(k-1)*N2_partition+1;
...
for k=N1_partition+1:N1_partition+N2_partition
    boundary_edges(2,k)=(N1_partition-1)*N2_partition+(k-N1_partition);
...
for k=N1_partition+N2_partition+1:2*N1_partition+N2_partition
    boundary_edges(2,k)=(2*N1_partition+N2_partition+1-k)*N2_partition;
...
for k=2*N1_partition+N2_partition+1:nbe
    boundary_edges(2,k)=2*N1_partition+2*N2_partition+1-k;
